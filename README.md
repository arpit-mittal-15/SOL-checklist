...
### Scheduled Email (4:00 PM IST Daily)
The app includes automatic email sending to `suckzhum@gmail.com` and `braj@thesolfactory.com` at 4:00 PM IST daily.
...
1. **Client-Side Check (Automatic)**: If the dashboard page is open, it will automatically check and send emails at 4:00 PM IST. However, this requires the page to be open.
...
2. **External Cron Service (Recommended)**: For reliable daily emails, set up an external cron service:
   ...
   - Schedule a GET request to: `https://your-domain.com/api/scheduled-email` at 4:00 PM IST (16:00 IST)
